...
**Changelog (2.9.0 → 3.0.0):**
- Phase B5 + B7 + demo wiring lands. The 3.0.0 bump signals Phase B complete and the architectural fix demonstrated end-to-end on both halves of the problem: negative case (Venkateshulu, pure dismissal -> one-card defensive SLP plan) and positive case (synthetic disposed-with-directions -> three-card ACTIVE_OBLIGATION plan with deadlines computed from grounded directives).
- Rules engine bumped to 0.3.0. New behavior on the directive-bearing branch of `generate_actions(...)`: when `case.directives` is non-empty, every grounded directive becomes one ACTIVE_OBLIGATION action with deadline computed from the directive's `time_clause` (calendar-day arithmetic per §10.2: WEEKS=7d, DAYS=1d, MONTHS=30d, YEARS=365d), `target_role_id` set to the resolved respondent FK, and `source_directive_id` set to the directive's index in `case.directives` so the validator's OBLIGATION_WITHOUT_SOURCE check passes. The Phase A pure-dismissal -> SLP-monitor branch is unchanged.
- New synthetic fixture `tests/fixtures/synthetic_disposed_with_directions/`. One-page hand-rolled PDF (no reportlab in §8 dependency set; the build script uses pure-Python PDF assembly with Times-Roman, WinAnsi encoding, fixed line height — pdfplumber reads it cleanly). Three respondents (State of Karnataka, Deputy Commissioner, Tahsildar), three directives in paragraph 6 with three different time-clause units (weeks, sixty days, ninety days), three actor references via ordinal language. Calibrated to exercise: cause-title parsing on a non-Venkateshulu format, segmentation on a 6-paragraph body, voice tagging on a party-contention paragraph, section classification reaching the new `directive-issuance-last` rule, directive extraction with three different time clauses, actor resolution via ordinal (3, 2, 1), and engine ACTIVE_OBLIGATION emission with three different `rule_id` values.
- New section classifier rule: `directive-issuance-last` -> OPERATIVE (confidence 0.95). Catches paragraphs whose text contains the directive-issuance phrase pattern (`(is|are) (hereby )?directed to`) AND `is_last_body_paragraph=True` AND `court_voice_ratio >= 0.95`. Without this rule, the synthetic positive case's paragraph 6 (which has no decree verb in the existing dismissal-pattern set) would route to UNCERTAIN and the LLM stub would commit it to REASONING, breaking the directive extraction path. Rule order: (1) cue+decree+court, (2) cue+decree+last, (2a NEW) directive-issuance-last, (3+) the existing rule chain unchanged. Venkateshulu's P24 still hits rule 1 first and is unaffected.
- Cause-title parser generalization: `_text_without_signature_bbox` now runs the bbox filter only when a signature marker (`Digitally`, `signed`, `Location:`, `NIRMALA DEVI`) is detected inside the bbox. Without this guard, the unconditional bbox strip ate left-margin numbered prefixes (`2.`, `3.`) on any non-Venkateshulu PDF whose body text happened to fall in the bbox y-range, and the respondent splitter merged all respondents into R1. The synthetic positive case is the canary that surfaced this; the fix is generalization beyond the Venkateshulu calibration.
- Phase B7: deprecation markers added to `OperativeDirection` (in `kartavya/schemas/extraction.py`) and `LegacyActionPlan` (in `kartavya/schemas/action_plan.py`). Both classes carry updated docstrings noting the replacement type and the 0.4.0 removal target. Constructing either emits a `DeprecationWarning` at stacklevel=2; pytest surfaces these (16 warnings emitted by the existing legacy 0.1.0 tests, none of which are failures). The legacy 0.1.0 integration test (skipped throughout Phase B per §5.2 acceptance) stays skipped; removal in 0.4.0 will delete it together with the legacy classes. v3 prompts retained on disk per §3.5.
- New module `kartavya/cli/` containing `run.py`. argparse-based CLI: `python -m kartavya.cli.run <pdf> [--dry-run] [--demo-positive] [--json] [--today YYYY-MM-DD]`. Wires the full Phase B pipeline (cause title -> segment -> voice -> section -> directives) into the Phase A engine and renders the resulting ActionPlan as a formatted table (default) or JSON (`--json`). `--dry-run` swaps in stubbed LLM clients (no Ollama dependency); `--demo-positive` injects the canned positive-case directives so `make demo-positive` works without requiring Ollama in the demo environment. Exit code is 0 if the plan validates cleanly and 1 if `validate_action_plan` returns errors.
- New Makefile targets: `make demo-venkateshulu` (negative case, --dry-run, today=2026-05-07) and `make demo-positive` (positive case, --dry-run --demo-positive, today=2026-03-15). Both run end-to-end without external dependencies; the user-facing terminal output is the demo. For real Ollama wiring (production path), the CLI's `_real_clients()` helper wraps the existing `OllamaClient` in a thin dict-returning adapter that matches the `SectionLLMClient` / `DirectiveLLMClient` Protocols defined in B2 and B4.
- New tests: 3 in `tests/integration/test_positive_case_full_pipeline.py` (metadata parsed correctly, paragraph 6 classified OPERATIVE, three active obligations with computed deadlines and zero validator errors). All Phase A through B4 tests still pass.
- One brief deviation: the brief's expected_action_plan.json titles like "Complete field verification of mutation application" assume an action sub-span extractor that B5 does not ship. v0.1's `_directive_title` takes the substring up to the first sentence-ending period or 100 characters, whichever is shorter; the resulting titles are verbose but accurate ("The third respondent is directed to complete field verification of the petitioner's application with..."). The integration test asserts deadlines, target_role_ids, and rule_ids, not titles. Refining titles to action-only is a Phase B6 task.
- One brief deviation: the brief's `_action_from_directive` interface had a deadline-computation shape that did not have access to `case.judgment_date`. Replaced with `_directive_action_in_case(directive, judgment_date, *, source_directive_id)` which is case-aware. The brief's intent is preserved: directive-driven ACTIVE_OBLIGATION emission with calendar-day arithmetic.
- One brief deviation: the brief assumes `make` is available. The Makefile target list is added regardless, but the CLI is the primary interface and works directly via `.venv/bin/python -m kartavya.cli.run ...`. Both demos verified by direct CLI invocation in the dev environment.
- No new dependencies. The synthetic PDF builder is pure Python (no reportlab). The CLI uses argparse from the stdlib. The Phase A through B4 test count (141 passed, 1 skipped) is unchanged; B5 adds 3 tests for the positive case path. New total: 144 passed, 1 skipped, 16 deprecation warnings (the warnings surface from legacy 0.1.0 tests still constructing OperativeDirection / LegacyActionPlan; this is the intended behavior of the deprecation markers).
- Pre-flight: 144 passed, 1 skipped. Ruff clean. Mypy clean across 39 source files. Test breakdown: Phase A 59 + B1+B6 18 + B3 22 + B2 24 + B4 18 + B5 3 = 144. Demo CLI verified end-to-end on both fixtures: Venkateshulu yields one DEFENSIVE_MONITOR card (deadline 2026-07-16, target R3 Principal Secretary Commerce & Industries); synthetic positive yields three ACTIVE_OBLIGATION cards (deadlines 2026-04-12, 2026-05-14, 2026-06-13; targets R3 Tahsildar, R2 Deputy Commissioner, R1 State of Karnataka).
- The architectural-fix demonstration in its complete form, on both halves of the problem: a compliance system that fabricates obligations on a dismissed case wastes officer time and erodes trust; the system instead produces exactly one defensive card. A directive-bearing judgment must produce one card per directive, with deadlines and targets grounded in the source PDF; the system produces exactly three, each pointing at the operative paragraph and the verbatim text the court used. Three independent grounding guards (section_class, voice_in_span, bounds + substring) plus the actor FK guard plus the verdict gate at the rules engine plus render-time validators (TARGET_NOT_IN_RESPONDENTS, OBLIGATION_WITHOUT_SOURCE, DISMISSED_WITH_OBLIGATION, SOURCE_PARAGRAPH_MISSING, SOURCE_PARAGRAPH_NOT_OPERATIVE, UNGROUNDED_PRIMARY_STATE_TARGET) make every wrong output route a closed door.

**Changelog (2.8.0 → 2.9.0):**
- Phase B4 lands. New module `kartavya/extraction/directives.py` is the span-only directive parser. The LLM emits character offsets only; the parser reconstructs `verbatim_text` from `paragraph.text[char_start:char_end]`. Three independent grounding guards (section_class, voice_in_span, bounds + substring) plus an actor FK guard make hallucinated or mis-located directives structurally unreachable. Public API: `extract_directives(case, *, llm_client, model)`, `resolve_actor(actor_text, respondents)`, `UNRESOLVED_RESPONDENT_NO=-1`, `DirectiveLLMClient` Protocol, `DIRECTIVE_PARSER_SCHEMA`.
- New v4 prompt at `kartavya/extraction/prompts/operative_extractor.v4.md` per §3.5. v1, v2, v3 retained on disk; runtime dispatch is solely v4 in B4. Unlike v3, v4 instructs the LLM that it points at offsets but never writes verbatim_text. The schema response shape is offsets + actor_text + verb_token + optional time_clause_text only — no free-form fields.
- Section guard runs upstream of the LLM call. Paragraphs whose section_class != "OPERATIVE" are skipped before the parser even renders a prompt. Wasted LLM calls on non-OPERATIVE paragraphs would cost money and surface area; the filter is mandatory and tested (`test_non_operative_paragraph_skipped`, `test_p21_revisional_quote_not_queried`).
- Voice guard runs at directive construction. Within an OPERATIVE paragraph, every directive's char range is checked against `paragraph.voice_in_span(s, e) == "COURT"`. Statutory paraphrases, party contentions, and quoted holdings are excluded. P21 of Venkateshulu (the source of the original phantom-cards bug, REASONING-class after B2) cannot reach this guard at all because of the section guard upstream; the voice guard exists for OPERATIVE paragraphs that contain a quote (rare but possible).
- Bounds + substring guard: every offset range must satisfy `0 <= s < e <= len(text)`, and `verbatim_text` is *constructed from* `text[s:e]` rather than passed in by the LLM. A hallucinated paraphrase cannot survive: either the offsets are out of bounds (rejected) or the offsets slice to a real substring (which is then the verbatim_text by construction).
- Actor FK guard. The LLM-returned `actor_text` is mapped to a respondent_no via three resolution strategies: (1) ordinal language ("the second respondent" -> 2, "respondent No.3" -> 3), (2) unique-token designation match (a token from the respondent's distinctive_tokens that is NOT shared with any other respondent's distinctive_tokens; "the Director of Mines" -> respondent_no=5 because 'director' is unique to R5 even though 'mines' is shared with R1, R2, R6), (3) organization fallback ("the State of Karnataka" -> lowest-numbered Karnataka respondent; "Union of India" -> lowest-numbered GoI respondent). Anything that cannot be grounded returns `UNRESOLVED_RESPONDENT_NO=-1`, which fails the FK check `actor_resolved in {r.respondent_no for r in case.respondents}` and the directive is rejected. UNRESOLVED is never silently substituted with a guess; it routes to human review.
- Per-directive validation failures are caught and audit-logged; the loop continues with the next candidate. One bad directive does not poison the rest. Hard LLM failures (unreachable client, schema rejection at the LLM layer) are propagated to the caller; B4 is not the right layer to decide retry-vs-fail policy.
- Audit recorder gains four event helpers (`directive_extraction_raw`, `directive_extraction_constructed`, `directive_extraction_rejected`, `directive_extraction_failure`) routed through the existing `record(...)` writer. Each helper builds a deterministic UUIDv5 entity_id from `(case_number, paragraph_index, "directive")`, satisfies §3.2 (prompt_sha implies non-empty paragraph_ids) on the events that carry a prompt_sha, and writes a `payload["stage"]` discriminator so the four kinds are filterable downstream.
- New fixture: `kartavya/extraction/prompts/operative_extractor.v4.md`. No new YAML rule tables.
- Tests: 13 in `tests/unit/test_directive_parser.py` (six actor resolver cases — ordinal word, ordinal number, unique-token success, ambiguous-fallthrough, state-of-karnataka org fallback, unknown-returns-UNRESOLVED — plus seven end-to-end-with-stub cases — pure dismissal, well-grounded directive, substring-mismatch reject, unresolved-actor reject, non-court-voice reject, non-OPERATIVE-paragraph skipped, one-bad-does-not-poison-good); 3 in `tests/integration/test_directives_real_pdf.py` (zero directives extracted from Venkateshulu, only paragraph 24 queried, paragraph 21 never queried); 2 in `tests/integration/test_full_pipeline_phase_b_with_directives.py` (full pipeline produces one-card SLP plan with deadline 2026-07-16 and zero validator errors, paragraph 24 has no directive verbs at all so phantom cards remain unreachable). Total Phase B4: 18 new tests.
- Two minor adjustments from the brief, both forced by the type system: (a) the brief's `_build_directive` raises `ValidationError` on guard failures, but Pydantic's grounding validators run on `ParsedJudgment.validate_directive_grounding`, not on `OperativeDirective` in isolation; replicating the guard logic in the parser and raising a private `_DirectiveRejected` exception is faster (no round-trip through ParsedJudgment construction) and keeps the rejection-reason text close to the failure site for audit logging; (b) `TimeClause.unit` is a `Literal[...]` and `_UNIT_MAP[unit_raw]` returns `str`, so a one-line `cast(Literal[...], ...)` was needed to satisfy mypy. Neither is a contract change; tests are unaffected.
- The architectural-fix demonstration in its complete form: the full Phase B pipeline (segmentation -> voice -> section -> directives) plus the Phase A 0.2.0 engine, run end-to-end against the real Venkateshulu PDF, produces the same one-card defensive SLP plan that the Phase A stub fixture produced. Three independent grounding guards plus the actor FK guard plus the verdict gate at the engine make the four historical phantom cards unreachable through any path. The closing test (`test_phantom_cards_remain_unreachable_through_full_pipeline`) pins the structural fact: paragraph 24, the only OPERATIVE paragraph in Venkateshulu, has no directive verbs at all (no "directed", "ordered", "shall", "within"). Even an LLM that hallucinated freely cannot point at a directive verb that exists in the source; the substring guard would catch any fabricated span.
- v0.1 limitations (documented inline in `directives.py`): multi-respondent directive ranges ("respondents 1 and 2 are directed to ...") return the first match and audit-log the range; splitting into N OperativeDirective instances is a v0.2 refinement. `object_text` is set to the full `verbatim_text` rather than the action sub-span. The actor resolver is conservative on designation matching: ambiguous matches return UNRESOLVED rather than guess. The brief's positive-case demonstration (a "disposed with directions" judgment with real directives) is deferred to B5; B4 ships the parser, B5 ships the fixture that proves the parser correctly extracts when directives exist.
- No new dependencies. v3 prompts and `OperativeDirection` retained per §3.5. No edits to Phase A schemas, the Phase A engine, the responsibility mapper, or any prior-phase test. New files: `kartavya/extraction/directives.py`, `kartavya/extraction/prompts/operative_extractor.v4.md`, `tests/unit/test_directive_parser.py`, `tests/integration/test_directives_real_pdf.py`, `tests/integration/test_full_pipeline_phase_b_with_directives.py`. Modified files: `kartavya/audit/recorder.py` (+4 helpers, +UUIDv5 imports).
- Pre-flight: 141 passed, 1 skipped (the legacy 0.1.0 integration test, unchanged from 2.8.0). Ruff clean. Mypy clean across 37 source files. Test breakdown: Phase A 59 + B1+B6 18 + B3 22 + B2 24 + B4 18 = 141.

**Changelog (2.7.0 → 2.8.0):**
- Phase B2 lands. New module `kartavya/extraction/section.py` populates `GroundedParagraph.section_class` via a two-stage classifier. Stage 1 is deterministic and consumes the voice-density features built from B3's `voice_spans`; stage 2 is an LLM fallback that runs only when stage 1 confidence falls below 0.70. The deterministic stage's rule set has a deliberate gap (between "court-quote dominant + court_voice_ratio < 0.30 -> PRECEDENT/REASONING by quote provenance" and "court-quote + moderate court voice 0.40 to 0.70 -> REASONING") that catches paragraph 21 of Venkateshulu (revisional-authority quote dominates the paragraph at 95% non-COURT) and routes it to UNCERTAIN. The LLM, given prev/next paragraph context, commits to REASONING. Both stages are recorded in the audit log when `case_number` is supplied.
- Public API: `SectionVerdict` (NamedTuple: `section_class`, `confidence`, `reason`, `stage`), `SectionFeatures`, `build_features`, `classify_deterministic`, `classify_with_fallback`, `classify_paragraphs`. The `SectionLLMClient` Protocol is the minimal interface for the LLM stage (one method: `generate_json(prompt: str) -> dict`); tests pass MagicMock, production wires up an Ollama-backed adapter.
- Eleven-rule deterministic classifier in priority order: (1) OPERATIVE cue + decree verb in court voice + court_ratio>=0.95; (2) OPERATIVE cue + decree + last paragraph; (3) ARGUMENTS contention opener; (4) PRECEDENT_CITATION statute-dominant (statute_chars/total>=0.50); (5a) PRECEDENT_CITATION Supreme Court quote dominant (external precedent); (5b) REASONING this-court-prior-order adopted (OTHER_COURT_QUOTE dominant — KHC writs follow such quotes with adoption clauses); (6) REASONING start-of-paragraph cue + court_ratio>=0.50; (7) REASONING "In the present case ..." applies-to-facts; (8) FACTS narrator cue; (9) REASONING court endorsement marker ("appropriately noticed", "warranting interference", "cannot be stated to be erroneous", "cannot be faulted"); (10) FACTS high court voice no quotes; (11) REASONING moderate court voice + court quote (below threshold; routes to LLM). All 24 Venkateshulu paragraphs classify correctly; only P21 is UNCERTAIN.
- Five rule deviations from the brief, each forced by sweeping the real PDF: (a) lowered statute-dominant threshold from 0.60 to 0.50 because P17 of the real PDF measures at 52% statute paraphrase; (b) split rule 5 by quote provenance — SUPREME_COURT_QUOTE dominant -> PRECEDENT_CITATION (external precedent), OTHER_COURT_QUOTE dominant -> REASONING (this court adopting its own prior order, canonical example P13 of Venkateshulu); (c) added a new "court-endorsement-marker" rule between the FACTS-cue and high-court-no-quotes rules, calibrated to fire on P23 ("The second respondent has appropriately noticed ... cannot be stated to be erroneous warranting interference") and to NOT fire on P15 ("untenable and liable to be rejected" is a different marker family); (d) tightened the operative-cue regex from `[Pp]etition` to `[Pp]etition\b` so paragraphs starting with "The petitioner" do not falsely match (P12 of the real PDF would otherwise be blocked from the FACTS catch-all); (e) removed `It is (also/further) pertinent` from the start-of-paragraph reasoning cue regex because it fires on factual-recital paragraphs (P14 and P15 of the real PDF) that the brief classifies as FACTS.
- Each deviation is justified empirically: if rule 5 had not been split, P13 would classify as PRECEDENT_CITATION (false negative on REASONING); if the endorsement-marker rule had not been added, P23 would default to FACTS (false negative on REASONING); without the operative-cue \b boundary, P12 would route to UNCERTAIN; without removing `pertinent` from reasoning cue, P14 and P15 would falsely classify as REASONING. After all five adjustments, only P21 reaches UNCERTAIN — exactly as the brief intends, and the LLM-fallback path is exercised by exactly one paragraph.
- New prompt `kartavya/extraction/prompts/section_classifier_v1.md`. YAML frontmatter (task, version, model, temperature, schema). Body explains the six classes, names the PRECEDENT_CITATION vs REASONING distinction (the load-bearing call), then injects voice analysis, paragraph index/total, prev/next previews, and the paragraph text. Schema-constrained: response is `{"section_class": "<one of six classes>"}`. UNCERTAIN is not in the schema; if the LLM returns an off-enum value or raises, the fallback path returns `(UNCERTAIN, DETERMINISTIC_LOW_CONFIDENCE)` with a structlog warning.
- Audit logging via existing `kartavya.audit.recorder.record(...)`. Caller passes optional `case_number`; when supplied, every classification (deterministic and LLM) emits one `EXTRACTION_COMPLETED` event on entity_type=PARAGRAPH with a deterministic UUIDv5 derived from `(case_number, paragraph_index)`. Payload carries final class+confidence+reason+stage and the deterministic class+confidence+reason; LLM-fallback events additionally carry `prompt_sha`, `model_id`, `temperature=0.0`. The §3.2 invariant (prompt_sha implies non-empty paragraph_ids) holds: `paragraph_ids=[para_uuid]` is provided whenever prompt_sha is. Without `case_number`, the classifier is hermetic — useful for unit tests that don't want to touch the audit store.
- New fixture `tests/fixtures/venkateshulu_real_pdf_wp13296_2022/expected_sections.json`. All 24 paragraphs with `section_class` and `expected_stage`. P21 is the only LLM_FALLBACK; everything else is DETERMINISTIC.
- Tests: 14 in `tests/unit/test_section_classifier.py` (one per deterministic rule plus the OTHER_COURT_QUOTE-dominant-is-REASONING test, the court-endorsement-marker test, the UNCERTAIN-when-no-rule-fires gap test, and the P21-shape-routes-to-UNCERTAIN load-bearing structural test); 8 in `tests/integration/test_section_real_pdf.py` (real-PDF sweep, only-P21-routes-to-LLM, P24-is-OPERATIVE, P21-resolved-to-REASONING, no-paragraph-remains-UNCERTAIN, LLM-unavailable-degrades-gracefully, audit-log-emits-one-event-per-paragraph); 2 in `tests/integration/test_full_pipeline_phase_b.py` (full Phase B pipeline assembles a valid `ParsedJudgment`, Phase A engine runs against it and produces the expected one-card SLP plan with deadline 2026-07-16). Total Phase B2: 24 new tests.
- The Phase A engine is untouched. The Phase B end-to-end test (`test_phase_a_engine_runs_against_phase_b_pipeline_output`) shows the architectural fix holds: real PDF in, voice-tagged classified paragraphs out, Phase A 0.2.0 engine produces the same one-card SLP plan that the synthetic stub fixture produced, with `validate_action_plan` returning zero errors.
- v0.1 limitations (documented inline in `section.py`): rule confidences are heuristic — calibration was empirical on Venkateshulu and may need adjustment on the second canonical case; the OTHER_COURT_QUOTE-dominant -> REASONING rule assumes adoption (true in KHC writ practice but not universal); the endorsement-marker family is intentionally narrow (six markers) to keep precision high. Adding markers should preserve "P15 stays FACTS" as a guard.
- No new dependencies. No edits to Phase A schemas, the Phase A engine, or any prior-phase prompts. `kartavya/extraction/section.py` and `kartavya/extraction/prompts/section_classifier_v1.md` are the only new files; existing files are unchanged.
- Pre-flight: 123 passed, 1 skipped (the legacy 0.1.0 integration test, unchanged from 2.7.0). Ruff clean. Mypy clean across 36 source files. Test breakdown: Phase A 59 + B1+B6 18 + B3 22 + B2 24 = 123.

**Changelog (2.6.0 → 2.7.0):**
- Phase B3 lands. New module `kartavya/extraction/voice.py` populates `GroundedParagraph.voice_spans` deterministically (no LLM). Four passes per paragraph in order: (1) block quotes anchored on an introducer phrase ("reads/held/is as under:", "as follows:") with attribution classified from a 200-char preamble (Supreme Court / revisional authority / this Court / default OTHER_COURT_QUOTE); (2) statutory paraphrase ("Section <N> of the <Act> stipulates|states|provides|prescribes|mandates|requires|empowers|defines that ..."); (3) party contention ("It is the contention of the petitioner...", "Per contra, the learned AGA...") tagging the paragraph after the first sentence as PARTY_CONTENTION; (4) sort-and-validate non-overlap. Public API: `tag_voice_spans(text)`, `annotate_paragraph(p)`, `annotate_paragraphs(ps)`.
- Quote-pairing handles both Unicode curly pairs (U+201C / U+201D distinct open/close) and ASCII straight quotes (U+0022 ambiguous). For straight quotes we use a "first-after-introducer = open, last-before-next-introducer = close" rule so embedded inner pairs (e.g. the `"first renewal"` paraphrase token inside the Common Cause quote in P20) are absorbed by the outer span rather than terminating it. The real KHC PDF rendering uses straight quotes throughout; the brief assumed curly. Bridge: synthetic unit tests exercise the curly branch; the real-PDF integration sweep exercises the straight branch.
- New fixture `tests/fixtures/venkateshulu_real_pdf_wp13296_2022/expected_voice_spans.json`. Substring-keyed (`starts_with` / `ends_with` per span) so re-segmentation whitespace shifts do not invalidate the fixture; the resolver maps to char offsets at runtime. Coverage: P9 (PARTY_CONTENTION), P11 (SUPREME_COURT_QUOTE), P13 (OTHER_COURT_QUOTE), P17 (one STATUTE_QUOTE), P18 (three STATUTE_QUOTE — Section 8A(3), 8A(6), 8A(9), the brief's outline named only the first two), P20 (SUPREME_COURT_QUOTE absorbing the inner `"first renewal"` pair), P21 (REVISIONAL_AUTHORITY_QUOTE).
- Tests: 11 in `tests/unit/test_voice_tagger.py` (synthetic, curly quotes, one per pass plus normalization and helper); 11 in `tests/integration/test_voice_real_pdf.py` (real PDF: per-paragraph span counts and content for P9, P11, P13, P17, P18, P20, P21, P24; cross-paragraph non-overlap; fixture substring resolution; the phantom-directive demonstration on P21 where `voice_in_span` returns REVISIONAL_AUTHORITY_QUOTE for the longest span's midpoint, structurally barring B4 from extracting a directive there). The brief's outline said 20 B3 tests; the +2 (P18 third statute span and the fixture-substring resolution test) come from verifying against the actual PDF rather than the outline.
- Documented v0.1 limitations: nested quotes not handled (outer span covers everything; B4 refuses extraction from the entire region anyway); unattributed block quotes default to OTHER_COURT_QUOTE rather than COURT (false-negative-safer); statutory paraphrase pattern restricted to "Section <N> of the <Act> <verb> that"; contention paragraphs that contain a Pass 1 block quote yield zero PARTY_CONTENTION spans (rather than a wrap-around list) — acceptable for canonical cases at hand.
- One brief deviation: brief's `_BLOCK_INTRODUCER` regex required a verb from {reads, held, observed, ...}, but P13 of the real PDF says "is as under:" (copula, not verb). Extended the verb set to include `is|are|was|were` so the copula form matches. No false positives observed in canonical paragraphs.
- One brief deviation: brief's quote-open / quote-close handling assumed distinct open/close characters; the real PDF uses straight quotes. Replaced single-pass `text.find(close, ...)` with the first-vs-last rule above, gated on whether the open character is straight or curly. Same outcome for curly-quote fixtures (unit tests); correct outcome for straight-quote fixtures (real PDF).
- No new dependencies. No edits to Phase A schemas, the Phase A engine, v3 prompts, or any earlier-phase files. `kartavya/extraction/voice.py` is a single new module.
- Pre-flight: 99 passed, 1 skipped. Ruff clean. Mypy clean across 35 source files. Test breakdown: Phase A 59 + B1+B6 18 + B3 22 = 99.

**Changelog (2.5.0 → 2.6.0):**
- Phase B1 + B6 land. Real-PDF ingestion replaces the synthetic Venkateshulu fixture for the 0.2.0 path. Bridge test: real PDF paragraph 24 text-equals the Phase A hand-typed stub paragraph 24, exactly.
- New module `kartavya/ingestion/segmentation.py`. Reads a KHC PDF; auto-detects the running header by line-frequency-across-pages; strips the running footer (`Indian Kanoon`) and page-number stamps (`-N-`); slices the body at "ORDER WAS PRONOUNCED AS UNDER" with fallback to the first `\n1.\s+[A-Z]` heading; splits on `\n(\d+)\.\s+(?=[A-Z\"])` with a strict-monotonic-increment filter that rejects spurious headings inside block quotes (e.g. quoted Supreme Court paragraph numbers). Strips the SD/-, judge-name-line, and court-initials signature block from the last paragraph only. `_assert_contiguous` raises `SegmentationError` if 1..N is broken. `section_class="FACTS"` placeholder; B2 will overwrite. `voice_spans=[]` placeholder; B3 will populate.
- New module `kartavya/ingestion/cause_title.py`. Reads pages 1-3; uses `extract_words()` to drop signature-block words from page 1 by bounding box (x0 < 150, top in [480, 600]) because pdfplumber's reading-order interleaves the signature column ("Digitally signed by NIRMALA DEVI Location: HIGH COURT OF KARNATAKA") with the respondent text; reconstructs page 1 text from the surviving words; concatenates pages 1-3, slices at "THIS WRIT PETITION IS FILED UNDER" or equivalent. Field extractors (deterministic regex, no LLM): case_number, judgment_date, court, petitioner_name, respondents. Multi-line designation join with prepositional and parenthetical heuristics ("Principal Secretary" + "to Government" -> space; "Senior Geologist" + "Department" -> comma). Address-token-based organization fallback (Vikasa Soudha, Bengaluru -> Government of Karnataka; Shastri Bhavan, New Delhi -> Government of India) when explicit organization phrase is absent. Returns `ParsedCauseTitle` (NamedTuple). `CauseTitleParseError` raised when any required field is missing. The §9 catalogue type `CaseMetadata` (Pydantic, Phase A) is unchanged; `ParsedCauseTitle` is a separate narrow tuple to avoid name shadowing.
- New fixture: `tests/fixtures/venkateshulu_real_pdf_wp13296_2022/` with `original.pdf` (the Indian Kanoon source), `expected_metadata.json`, `expected_respondents.json`, `expected_paragraph_24_text.txt`, `expected_paragraph_count.txt`, `README.md`. The respondents' designation strings match the Phase A stub exactly so the bridge test holds.
- Tests: 9 in `tests/unit/test_segmentation.py`, 7 in `tests/unit/test_cause_title.py`, 2 in `tests/integration/test_real_pdf_ingestion.py`. The bridge test `test_real_pdf_paragraph_24_matches_phase_a_stub_text` is the Phase B1 + B6 acceptance signal.
- What is Venkateshulu / KHC-specific and may need generalization (flagged for the future): the body-start markers list is calibrated to KHC writs; the signature-block bbox on page 1 of an Indian Kanoon rendering is hard-coded x-and-y ranges; the known-organization phrase set covers the Union and Karnataka only; the address-token mapping is calibrated to Karnataka secretariat geography. Generalization to other High Courts and order types wants either an LLM-assisted extract-then-validate pass or a small manually-tagged corpus for regex tuning. Documented as v0.1 limitations; not a Phase B blocker.
- One brief deviation: the brief's `_strip_signature_block` regex (`Digitally\s+signed\s+by[\s\S]+?HIGH\s+COURT\s+OF\s+KARNATAKA`) does not match the actual Indian Kanoon rendering, where pdfplumber's reading-order interleaves the signature column with respondent text ("Digitally ROOM No.320\nsigned by A WING..."). Replaced with bbox-based filtering. Same outcome (signature stripped), different mechanism, no functional change.
- One brief deviation on whitespace normalization: the brief's algorithm step 8 said preserve newlines for B3's voice tagger to use; the brief's test 5 (`test_paragraph_11_contains_supreme_court_quote`) is a substring check that fails when the substring spans a newline in pdfplumber's line-wrapped output. Going with the test-friendly choice (collapse all whitespace to single spaces). B3 finds quoted blocks via the `"..."` markers in the original text, which is the more reliable signal anyway.
- No new dependencies. pdfplumber already in §8.
- Pre-flight: 77 passed, 1 skipped (the legacy 0.1.0 integration test, unchanged from 2.5.0). Ruff clean. Mypy clean across 29 source files.
